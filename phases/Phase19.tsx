...
import MaterialVarianceAudit from '../components/Gameplay/MaterialVarianceAudit';
import PhaseResult from '../components/Gameplay/PhaseResult';
import TutorialOverlay from '../components/UI/TutorialOverlay';
import { PHASE19_DIALOGUE, PHASE19_DATA } from '../data/phase19';
import { TUTORIALS, ASSETS } from '../constants';

const Phase19: React.FC = () => {
    const { state, dispatch } = useGame();
    const lang = state.language;

    const [subPhase, setSubPhase] = useState<'DIALOGUE' | 'TUTORIAL' | 'GAME' | 'RESULT'>('DIALOGUE');
    const [currentNodeId, setCurrentNodeId] = useState<string>('start');
    const [currentNode, setCurrentNode] = useState<DialogueNode>(PHASE19_DIALOGUE[lang]['start']);
    const [score, setScore] = useState(0);

    const [bgImage, setBgImage] = useState(state.assets[PHASE19_DIALOGUE[lang]['start'].backgroundImage!] || ASSETS.BG_PUMP_ROOM);

    useEffect(() => {
        if (PHASE19_DIALOGUE[lang][currentNodeId]) {
            setCurrentNode(PHASE19_DIALOGUE[lang][currentNodeId]);
        }
    }, [currentNodeId, lang]);
    ...
    const handleChoice = (nextId: string, action?: (dispatch: any) => void) => {
        ...
        if (nextId === 'START_GAME') {
            ...
            dispatch({ type: 'SET_PHASE', payload: GamePhase.PHASE_20_LABOR_VOH_VAR });
        } else {
...
requiredBudget: c.requiredBudget
            ...
            {subPhase === 'TUTORIAL' && (
                <TutorialOverlay
                    title={TUTORIALS.PHASE_19?.title || "TUTORIAL: MATERIAL VARIANCES"}
                    steps={TUTORIALS.PHASE_19?.steps || []}
                    onComplete={() => setSubPhase('GAME')}
                />
            )}

            {subPhase === 'GAME' && (
                <MaterialVarianceAudit
                    scenario={PHASE19_DATA}
                    onCorrect={() => dispatch({ type: 'UPDATE_BUDGET', payload: 350 })}
            ...
            {subPhase === 'RESULT' && (
                <PhaseResult
                    ...
                    score={score}
                    maxScore={10}
                    feedbackText={score > 0 ? "Supply chain inefficiency exposed." : "Inventory levels remain unbalanced."}
                    tipText="Material Price Variance is calculated on Actual Quantity, while Usage Variance is calculated on Standard Price."
