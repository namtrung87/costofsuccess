...
import MarginOfSafetyPitch from '../components/Gameplay/MarginOfSafetyPitch';
import PhaseResult from '../components/Gameplay/PhaseResult';
import TutorialOverlay from '../components/UI/TutorialOverlay';
import { PHASE16_DIALOGUE, PHASE16_DATA } from '../data/phase16';
import { TUTORIALS, ASSETS } from '../constants';

const Phase16: React.FC = () => {
    const { state, dispatch } = useGame();
    const lang = state.language;

    const [subPhase, setSubPhase] = useState<'DIALOGUE' | 'TUTORIAL' | 'GAME' | 'RESULT'>('DIALOGUE');
    const [currentNodeId, setCurrentNodeId] = useState<string>('start');
    const [currentNode, setCurrentNode] = useState<DialogueNode>(PHASE16_DIALOGUE[lang]['start']);
    const [score, setScore] = useState(0);

    const [bgImage, setBgImage] = useState(state.assets[PHASE16_DIALOGUE[lang]['start'].backgroundImage!] || ASSETS.BG_COMMAND_CENTER);

    useEffect(() => {
        if (PHASE16_DIALOGUE[lang][currentNodeId]) {
            setCurrentNode(PHASE16_DIALOGUE[lang][currentNodeId]);
        }
    }, [currentNodeId, lang]);
    ...
    const handleChoice = (nextId: string, action?: (dispatch: any) => void) => {
        ...
        if (nextId === 'START_GAME') {
            ...
            dispatch({ type: 'SET_PHASE', payload: GamePhase.VICTORY }); // or a specific S2 END screen
        } else {
    ...
    const handleGameCorrect = () => {
        dispatch({ type: 'UPDATE_BUDGET', payload: 1000 }); // Massive boss fight reward
    };

    const handleGameIncorrect = () => {
        dispatch({ type: 'UPDATE_SANITY', payload: -40 }); // Brutal penalty
    };
        ...
        <div className="w-full h-full relative">
            <div
                ...
                style={{
                    ...
                    filter: 'brightness(0.3) saturate(1.2) contrast(1.2)' // Intense lighting for boss room
                }}
            />
            ...
            {subPhase === 'TUTORIAL' && (
                <TutorialOverlay
                    title={TUTORIALS.PHASE_16?.title || "BOSS FIGHT: MARGIN OF SAFETY"}
                    steps={TUTORIALS.PHASE_16?.steps || []}
            ...
            {subPhase === 'RESULT' && (
                <PhaseResult
                    ...
                    score={score}
                    maxScore={10}
                    feedbackText={score > 0 ? "Target accepted. Production authorized." : "Target rejected. Safety Net insufficient."}
                    tipText="Margin of Safety = (Target Sales - Break-Even Sales) / Target Sales. It's the percentage revenue can fall before the company starts losing money."
