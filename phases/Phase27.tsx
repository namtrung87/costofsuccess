...
import OptimizationGrid from '../components/Gameplay/OptimizationGrid';
import PhaseResult from '../components/Gameplay/PhaseResult';
import TutorialOverlay from '../components/UI/TutorialOverlay';
import { PHASE27_DIALOGUE, PHASE27_DATA } from '../data/phase27';
import { TUTORIALS, ASSETS } from '../constants';

const Phase27: React.FC = () => {
    const { state, dispatch } = useGame();
    const lang = state.language;

    const [subPhase, setSubPhase] = useState<'DIALOGUE' | 'TUTORIAL' | 'GAME' | 'RESULT'>('DIALOGUE');
    const [currentNodeId, setCurrentNodeId] = useState<string>('start');
    const [currentNode, setCurrentNode] = useState<DialogueNode>(PHASE27_DIALOGUE[lang]['start']);
    const [score, setScore] = useState(0);

    const [bgImage, setBgImage] = useState(state.assets[PHASE27_DIALOGUE[lang]['start'].backgroundImage!] || ASSETS.BG_OPS_CENTER);

    useEffect(() => {
        if (PHASE27_DIALOGUE[lang][currentNodeId]) {
            setCurrentNode(PHASE27_DIALOGUE[lang][currentNodeId]);
        }
    }, [currentNodeId, lang]);
    ...
    const handleChoice = (nextId: string, action?: (dispatch: any) => void) => {
        ...
        if (nextId === 'START_GAME') {
            ...
            dispatch({ type: 'SET_PHASE', payload: GamePhase.PHASE_28_MAKE_BUY });
        } else {
...
requiredBudget: c.requiredBudget
            ...
            {subPhase === 'TUTORIAL' && (
                <TutorialOverlay
                    title={TUTORIALS.PHASE_27?.title || "TUTORIAL: LIMITING FACTORS"}
                    steps={TUTORIALS.PHASE_27?.steps || []}
            ...
            {subPhase === 'RESULT' && (
                <PhaseResult
                    ...
                    score={score}
                    maxScore={10}
                    feedbackText={score > 0 ? "Resources balanced. High-priority items prioritized." : "Bottleneck warning: You are of producing the wrong products."}
                    tipText="Always prioritize by Contribution per Limiting Factor, not just total profit."
