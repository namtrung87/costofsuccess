    ...
    playSFX(key: 'HOVER' | 'CLICK' | 'SUCCESS' | 'ERROR' | 'SWIPE' | 'VALVE' | 'LAUNCH' | 'PICKUP' | 'DROP' | 'TYPE') {
        ...
        switch (key) {
            ...
            case 'ERROR':
                ...
                break;
            case 'SWIPE': {
                // White noise burst with bandpass sweep (200ms)
                const bufferSize = 0.2 * this.audioContext.sampleRate;
                const buffer = this.audioContext.createBuffer(1, bufferSize, this.audioContext.sampleRate);
                const data = buffer.getChannelData(0);
                ...
                const swipeGain = this.audioContext.createGain();
                swipeGain.gain.setValueAtTime(0.1, now);
                swipeGain.gain.linearRampToValueAtTime(0, now + 0.2);
                noise.connect(sweepFilter);
                sweepFilter.connect(swipeGain);
                swipeGain.connect(this.masterGain);
                noise.start(now);
                return;
            }
            case 'VALVE':
                // Low-frequency pulse with release (150ms)
                osc.type = 'square';
                ...
                break;
            case 'LAUNCH':
                // Rising sine sweep 200→1000Hz (300ms)
                osc.type = 'sine';
                osc.frequency.setValueAtTime(200, now);
