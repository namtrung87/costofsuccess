...
const PhaseResult: React.FC<PhaseResultProps> = ({
    score, maxScore, title, feedbackText, tipText, takeawayText, onContinue, onRetry, onPractice
}) => {
    const { state } = useGame();
    ...
    return (
        ...
        `} onClick={() => isSuccess && setShowReceipt(true)}>
                ...
                <div className="w-full bg-black/40 p-4 rounded border border-gray-700">
                    ...
                    {/* Key Takeaway (Tier 4) */}
                    {takeawayText && (
                        <div className="bg-neonCyan/10 border-l-4 border-neonCyan p-3 mt-4 text-left">
                            <div className="text-neonCyan font-black text-[10px] uppercase tracking-tighter mb-1">LEARNING_KEY // TAKEAWAY</div>
                            <p className="text-white text-xs font-mono">{takeawayText}</p>
                        </div>
                    )}
                </div>
