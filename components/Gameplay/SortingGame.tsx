...
import { SortingItem, CostCategory } from '../../data/phase2';
...
const SortingGame: React.FC<SortingGameProps> = ({ items, onComplete, onCorrect, onIncorrect }) => {
  ...
  const handleSwipe = (direction: 'LEFT' | 'RIGHT') => {
    ...
    // Logic: LEFT = PRIME, RIGHT = PERIOD
    const selectedCategory: CostCategory = direction === 'LEFT' ? 'PRIME' : 'PERIOD';
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md overflow-hidden">
      ...
      <div className="absolute top-16 md:top-20 text-center w-full">
        <h2 className="text-xl md:text-2xl font-heading text-white tracking-widest uppercase mb-1">Invoice Sorter</h2>
      ...
      <div className="relative w-full max-w-md h-[400px] flex items-center justify-center">
        ...
        `}>
            ...
            {swipeDirection && (
                <div className={`
                    absolute top-1/4 transform -rotate-12 border-4 font-black text-4xl uppercase p-2 z-50 opacity-0 animate-[stamp_0.2s_ease-out_forwards]
                    ${swipeDirection === 'LEFT' ? 'border-neonCyan text-neonCyan' : 'border-neonPink text-neonPink'}
                `}>
                    {swipeDirection === 'LEFT' ? 'PRIME' : 'PERIOD'}
                </div>
            )}

            {/* Card Content - Positioned within the frame image logic */}
        ...
        {/* Feedback Text (appears below card after swipe) */}
        {feedbackText && (
      ...
      <div className="absolute bottom-10 md:bottom-16 flex gap-8 items-center">
        ...
        >
            ...
            <span className="mt-2 font-heading font-bold text-neonCyan text-sm tracking-widest">PRIME</span>
            <span className="text-[10px] text-gray-500">Product Cost</span>
        </button>
        ...
        >
            ...
            <span className="mt-2 font-heading font-bold text-neonPink text-sm tracking-widest">PERIOD</span>
